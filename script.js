...
class ParallaxController {
    ...
    updateParallax() {
        ...
        const rate = scrolled * -0.1; // 0.9x speed (inverted for upward movement)
...
// Enhanced Contact Section Fade-in
...
// Enhanced Pixel Assistant with boundary detection
class EnhancedPixelAssistant extends PixelAIAssistant {
    constructor() {
        super();
        this.contentMargins = { left: 0, right: 0 };
        this.updateContentMargins();
        window.addEventListener('resize', () => this.updateContentMargins());
    }
    ...
    updateContentMargins() {
        ...
        if (mainContent) {
            ...
            this.contentMargins.left = rect.left;
            this.contentMargins.right = window.innerWidth - rect.right;
        }
    }

    setRandomPosition() {
        const margin = this.contentMargins.left + 50;
        const maxX = window.innerWidth - this.contentMargins.right - 50;
    ...
    moveTo(x, y) {
        const margin = this.contentMargins.left + 25;
        const maxX = window.innerWidth - this.contentMargins.right - 50;
        
        this.targetX = Math.max(margin, Math.min(maxX, x));
    ...
    showMessage(message) {
        ...
        // Enhanced boundary detection for speech bubble
        const bubbleRect = this.speechBubble.getBoundingClientRect();
        const assistantRect = this.assistant.getBoundingClientRect();
        
        let leftOffset = -50;
        
        // Check if bubble would go off left edge
        if (assistantRect.left + leftOffset < 10) {
            leftOffset = 10 - assistantRect.left;
        }
        
        // Check if bubble would go off right edge
        if (assistantRect.left + leftOffset + 140 > window.innerWidth - 10) {
            leftOffset = window.innerWidth - assistantRect.left - 150;
        }
        
        this.speechBubble.style.left = leftOffset + 'px';
...
class ButtonEffects {
    ...
    init() {
        ...
        const buttons = document.querySelectorAll('.cta-button, .filter-btn, .connect-pill');
        ...
        buttons.forEach(button => {
            button.addEventListener('mouseenter', (e) => {
                ...
                button.addEventListener('mousemove', this.magneticMove);
                button.addEventListener('mouseleave', this.magneticReset);
            });
        });
    }
    ...
    magneticMove = (e) => {
        ...
        const limitedX = Math.max(-5, Math.min(5, deltaX));
        const limitedY = Math.max(-5, Math.min(5, deltaY));
        
        button.style.transform = `translate(${limitedX}px, ${limitedY}px) translateY(-3px)`;
    }

    magneticReset = (e) => {
        const button = e.currentTarget;
        button.style.transform = '';
        button.removeEventListener('mousemove', this.magneticMove);
        button.removeEventListener('mouseleave', this.magneticReset);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Replace pixel assistant with enhanced version after delay
    setTimeout(() => {
        if (window.pixelAI) {
            const enhanced = new EnhancedPixelAssistant();
            window.pixelAI = enhanced;
        }
    }, 31000); // After original assistant appears
});