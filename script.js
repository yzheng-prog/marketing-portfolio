// Project data with new WHY/STRATEGY/ACTION/RESULT/BUSINESS INSIGHT structure
const projects = [
    ...
    // Project 3: VosynVerse AI Agile Transformation (UNCHANGED)
    {
        ...
        insight: "Applied FMCG campaign management discipline to tech startup environmentâ€”bringing enterprise-level process rigor while maintaining startup agility and speed.",
...
const modalIndustryTag = document.querySelector('.modal-industry-tag');
...
const modalInsightContent = document.querySelector('.modal-insight-content');
const modalTakeawayContent = document.querySelector('.modal-takeaway-content');
const modalInsightSection = document.querySelector('.modal-insight');
const modalTakeawaySection = document.querySelector('.modal-takeaway');
...
function formatProjectContent(content) {
    ...
    return content
        // Convert Phase labels to highlighted sections
        .replace(/Phase \d+ â€”([^:]+):/g, '<strong>Phase $1:</strong>')
        .replace(/Layer \d+ â€”([^:]+):/g, '<strong>Layer $1:</strong>')

        // Add spacing around major sections
        .replace(/\n\n/g, '</p><p>')

        // Highlight key strategic terms
        .replace(/\b(Value-Based Bidding|Offline Conversion Tracking|Custom Label Architecture|Demand Gen|First-Party Data)\b/g, '<span class="highlight-term">$1</span>')

        // Format bullet-style content
...
function populateModal(index) {
    ...
    modalIndustryTag.textContent = project.industry;

    // Check if this is a new framework project (has WHY/STRATEGY/ACTION/RESULT/BUSINESS INSIGHT)
    if (project.why) {
        // New framework structure: WHY â†’ STRATEGY â†’ ACTION â†’ RESULT â†’ BUSINESS INSIGHT
        ...
        // Old structure for VosynVerse (challenge/approach/execution/results/insight/takeaway)
        modalChallenge.innerHTML = `<p>${project.challenge}</p>`;
        modalApproach.innerHTML = `<p>${project.approach}</p>`;

        // Execution list
        modalExecution.innerHTML = '<ul>' +
            project.execution.map(item => `<li>${item}</li>`).join('') +
            '</ul>';

        // Results list
        modalResults.innerHTML = '<ul>' +
        ...
        if (project.insight) {
            ...
            modalInsightContent.innerHTML = `<p>${project.insight}</p>`;
        } else {
            modalInsightSection.style.display = 'none';
        }

        // Takeaway
        modalTakeawayContent.innerHTML = `<p>${project.takeaway}</p>`;
    }
}
...
class ProfessionalCursor {
    ...
    addHoverEffects() {
        const interactiveElements = document.querySelectorAll('a, button, .project-card, .timeline-node');
...
class CleanTimelineAnimation {
    ...
    observeAboutElements() {
        ...
        this.aboutElements.forEach(element => {
    ...
    setupSmoothScroll() {
        ...
        this.container.addEventListener('wheel', (e) => {
            // Only hijack if scrolling horizontally or if we need to scroll horizontally
            const maxScroll = this.container.scrollWidth - this.container.clientWidth;
            const atStart = this.container.scrollLeft === 0;
            const atEnd = this.container.scrollLeft >= maxScroll - 1;
...
class PixelAIAssistant {
    ...
    init() {
        // Wait 30 seconds before appearing
        setTimeout(() => {
    ...
    show() {
        ...
        this.jump();
        setTimeout(() => {
            this.showMessage("Hi! This site was built with Claude Code! ðŸ¤–");
        }, 1000);
    }
    ...
    setRandomPosition() {
        ...
        this.currentX = Math.random() * (maxX - margin) + margin;
        this.currentY = Math.random() * (maxY - margin) + margin;
    ...
    moveTo(x, y) {
        this.targetX = Math.max(25, Math.min(window.innerWidth - 50, x));
        this.targetY = Math.max(25, Math.min(window.innerHeight - 100, y));
        this.isMoving = true;
    ...
    runAway() {
        ...
        this.speed = 2; // Faster when running
    ...
    startBehaviorLoop() {
        const behaviorLoop = () => {
            ...
            if (timeSinceLastAction > 5000 && !this.isMoving) {
