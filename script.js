...
const projects = [
    ...
    // Project 3: VosynVerse AI Agile Transformation (UNCHANGED)
    {
        ...
        insight: "Applied FMCG campaign management discipline to tech startup environment—bringing enterprise-level process rigor while maintaining startup agility and speed.",
...
function formatProjectContent(content) {
    ...
    return content
        ...
        .replace(/Layer \d+ —([^:]+):/g, '<strong>Layer $1:</strong>')
...
function populateModal(index) {
    ...
    if (project.why) {
        // New framework structure: WHY → STRATEGY → ACTION → RESULT → BUSINESS INSIGHT
...
class PixelAIAssistant {
    ...
    show() {
        ...
        this.jump();
        setTimeout(() => {
            this.showMessage("Hi! This site was built with Claude Code! 🤖");
        }, 1000);
    }
